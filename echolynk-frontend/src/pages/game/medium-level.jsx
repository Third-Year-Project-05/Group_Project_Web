...
import { getAllGames } from "../../services/gameService"; // Adjust the path if needed
  ...
  useEffect(() => {
    const fetchGames = async () => {
      try {
        ...
        const mediumGames = allGames.filter((game) => game.gameLevel === "medium"); // Filter for medium level
        setGames(mediumGames);
      } catch (err) {
    ...
    <div className="min-h-screen bg-blue-900 text-white flex items-center justify-center px-4 py-8">
      <div className="text-center">
        ...
        {!loading && !error && games.length === 0 && (
          <p className="text-gray-300">No medium-level games available.</p>
        )}
        <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 justify-center">
          {games.map((game, index) => (
            ...
            >
              ...
              <div
                onClick={() => handlePlay(game.id)} // Assuming game.id is the correct property
