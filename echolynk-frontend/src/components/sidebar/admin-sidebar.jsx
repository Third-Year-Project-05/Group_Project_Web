        ...
        <div className={`${theme == 'dark' ? 'sidebar-dark' : 'sidebar-light'} flex `}>
            <div className={`relative flex flex-col  shadow-lg transition-width duration-300 ${isOpen ? 'w-64' : 'w-20'}`}>
                ...
                <div className="mt-auto p-4 border-t border-gray-300">
                    <Link to="/settings" className={`flex items-center p-2 rounded-md ${isCurrentPage('/settings') ? 'bg-blue-900 text-white' : 'hover:bg-blue-900 hover:text-white'}`}>
                        <CogIcon className="h-6 w-6" />
                        {isOpen && <span className="ml-4">Settings</span>}
                    </Link>
                    <Link to="/logout" className={`flex items-center p-2 rounded-md mt-4 ${isCurrentPage('/logout') ? 'bg-blue-900 text-white' : 'hover:bg-blue-900 hover:text-white'}`}>
